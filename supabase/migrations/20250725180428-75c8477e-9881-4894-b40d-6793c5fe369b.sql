-- First, identify and clean up duplicate payment IDs
...
DELETE FROM orders 
WHERE id IN (
  SELECT unnest(order_ids[2:]) -- Keep first, delete rest
...
-- Now add unique constraint on payment_id to prevent future duplicates
ALTER TABLE orders ADD CONSTRAINT unique_payment_id UNIQUE (square_payment_id);

-- Create database transaction function for atomic order creation
